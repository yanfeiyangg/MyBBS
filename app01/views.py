...
def reg(request):
    try:
        ...
        print("reg - 注册，服务器逻辑错误：", e)
        return JsonResponse({
            "error": "reg - 注册，服务器逻辑错误：" + str(e),
            "is_send": 0
        })
...
def sendVaildEmail(request):
    try:
        ...
        return JsonResponse({
            "statue": 0,
            "error": "sendVaildEmail - 发送邮箱验证码,服务器逻辑错误:" + str(e)
        })
...
def article_detail(request, username, article_id):
    try:
        ...
        print("article_detail - 文章详细页，服务器逻辑错误：", e)
        return render(request, "500.html")
...
def poll(request):
    try:
        ...
        print("poll - 点赞，服务器逻辑错误：", e)
        return JsonResponse({
            "is_up": "error",
            "error": "poll - 点赞，服务器逻辑错误：" + str(e)
        })
...
def comment(request):
    try:
        ...
        print("comment - 评论，服务器逻辑错误：", e)
        return JsonResponse({
            "state": False,
            "error": "comment - 评论，服务器逻辑错误：" + str(e)
        })
...
def backend(request):
    try:
        ...
        print("backend - 后台管理界面，服务器逻辑错误：", e)
        return render(request, "500.html")
    ...
    try:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                print("add_article - 添加文章保存数据库步骤错误：", e)
                return render(request, "400.html")
            return redirect("/blog/backend/")
    except Exception as e:
        print("add_article - 添加文章，服务器逻辑错误：", e)
        return render(request, "500.html")
...
def delete_article(request, article_id):
    try:
        if request.method == "GET":
            ...
            if article:
                article.delete()
                v.delView(article_id)
                # # article.delete()
                return JsonResponse({"status": "0"})
            else:
                return JsonResponse({
                    "status": "1",
                    "error": "delete_article - 删除文章，逻辑错误"
                })
    except Exception as e:
        print("delete_article - 删除文章，逻辑错误:", e)
        return JsonResponse({"status": "1", "error": "delete_article - 删除文章，逻辑错误:" + str(e)})
        ...
        if request.method == "POST":
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    # 更改文章详细内容
                    articleDetail = models.ArticleDetail.objects.filter(article=article).update(content=str(bs))
                    # 先删除原标签，添加新标签
                ...
                print("edit_article - 编辑文章，数据库错误：", e)
                return render(request, "400.html")
            return redirect("/blog/" + username + "/articles/" + article_id)
    except Exception as e:
        print("edit_article - 编辑文章,逻辑错误：", e)
        return render(request, "500.html")
...
def upload(request):
    try:
        ...
        print("upload - 富文本编译器上传文件错误：", e)
        res = {
            "error": 0,
            "msg": "upload - 富文本编译器上传文件错误" + str(e)
        }
        return JsonResponse(res)
...
def getAvatar(request):
    try:
        ...
        print("getAvatar - ajax请求头像，逻辑错误：", e)
        return JsonResponse({"error": "getAvatar - ajax请求头像，逻辑错误：" + str(e)})
...
def add_category(request):
    try:
        ...
        try:
            with transaction.atomic():
                models.Category.objects.create(title=cate, blog=request.user.blog)
                data["state"] = 1
        except Exception as e:
            data = {"state": 0}
            print(e)
        ...
        print("add_category - 添加分类，逻辑错误:", e)
        return JsonResponse({"state": 0, "error": "add_category - 添加分类，逻辑错误" + str(e)})
...
def add_tag(request):
    try:
        ...
        print("add_tag - 添加标签，逻辑错误：", e)
        return JsonResponse({"state": 0, "error": "add_tag - 添加标签，逻辑错误" + str(e)})
